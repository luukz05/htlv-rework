  ...
  return (
    <main>
      <section className="relative overflow-hidden border-b border-border bg-bg-body">
        {featuredProfile.heroImage && (
          <>
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-r from-bg-body via-bg-body/90 to-bg-body/45" />
            <div className="absolute inset-0 bg-gradient-to-t from-bg-body via-transparent to-bg-body/20" />
          </>
        )}

        <div className="relative mx-auto max-w-[1460px] px-4 py-6 sm:px-5 md:py-8">
          ...
          <div className="grid gap-7 xl:grid-cols-[minmax(0,0.95fr)_minmax(420px,0.55fr)]">
            <div>
              ...
              <div className="mt-6 border-l border-yellow/35 pl-5">
                ...
                <p className="mt-3 max-w-3xl text-sm leading-6 text-text-secondary md:text-[15px]">
      ...
      <section className="border-b border-border bg-bg-surface/20">
        <div className="mx-auto grid max-w-[1460px] gap-7 px-4 py-8 sm:px-5 lg:grid-cols-[minmax(0,1fr)_390px]">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-text-muted">
              Legacy Biography
            </p>
            ...
            <div className="mt-4 columns-1 gap-8 lg:columns-2">
              {featuredProfile.biography.map((paragraph, index) => (
                <p key={index} className="mb-4 break-inside-avoid text-sm leading-7 text-text-secondary">
                  {paragraph}
                </p>
              ))}
            </div>
          ...
          <div className="grid content-start gap-6">
            <section>
              <h3 className="border-b border-yellow/20 pb-2 text-sm font-bold uppercase tracking-[0.2em] text-yellow">
                Signature moments
              </h3>
            ...
            <section>
              <h3 className="border-b border-blue/20 pb-2 text-sm font-bold uppercase tracking-[0.2em] text-blue-light">
                Career achievements
              </h3>
      ...
      <section className="mx-auto max-w-[1460px] px-4 py-8 sm:px-5">
        ...
        <div className="grid gap-x-8 gap-y-7 xl:grid-cols-2">
          {archiveProfiles.map((profile) => (
            <article key={profile.slug} className="border-t border-border pt-5">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {profile.achievements.map((achievement) => (
                      <span
                        key={achievement}
                        className="rounded-full border border-yellow/15 bg-yellow/5 px-3 py-1.5 text-xs text-text-primary"
                      >
                        {achievement}
                      </span>
                    ))}
      ...
      <section id="hltv-hall" className="border-t border-border bg-bg-surface/20">
        <div className="mx-auto max-w-[1460px] px-4 py-8 sm:px-5">
          ...
          <div className="mt-6 grid gap-x-8 gap-y-6 xl:grid-cols-2">
            {hltvHallOfFameClasses.map((hallClass) => (
              <article key={hallClass.year} className="border-t border-border pt-5">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-yellow">
                      {hallClass.classLabel}
                    </p>
                    <h3 className="mt-1 text-xl font-black md:text-2xl">{hallClass.year}</h3>
                  </div>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="border-l border-border pl-3">
                    ...
                    <p className="mt-1 text-sm font-semibold">{hallClass.announcementDate}</p>
                  </div>
                  <div className="border-l border-border pl-3">
                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-text-muted">
                      Induction ceremony
                    </p>
                    <p className="mt-1 text-sm font-semibold">{hallClass.inductionDate}</p>
                  </div>
                </div>
              </article>
            ))}
          ...
          <div className="mt-7 grid gap-x-8 gap-y-6 xl:grid-cols-2">
            {hltvHallOfFameBallots.map((ballot) => (
              <article key={ballot.year} className="border-t border-border pt-5">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-blue-light">
                      Nominee ballot
                    </p>
                    <h3 className="mt-1 text-xl font-black md:text-2xl">{ballot.year}</h3>
                  </div>
                ...
                <div className="mt-4 border-l border-border pl-3">
                  ...
                  <p className="mt-1 text-sm font-semibold">{ballot.ballotDate}</p>
                </div>
              </article>
            ))}
          </div>
